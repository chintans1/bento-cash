...
export default function ImportAccountsScreen({ navigation }) {
  ...
  return (
    <SafeAreaView style={{ flex: 1 }}>
      ...
      >
        <SectionList
          style={[oldCommonStyles.list, { marginTop: 10, marginBottom: 0 }]}
          ItemSeparatorComponent={separator}
          sections={getGroupedAccountsForImport(importData)}
          renderSectionHeader={({ section: { title: institutionName } }) => (
            <Text style={oldCommonStyles.sectionHeader}>{institutionName}</Text>
          )}
          renderItem={({ item }) => {
            if ('id' in item) {
              return <AccountComponent account={item} />;
            }
            return (
